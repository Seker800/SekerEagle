...
if (measureMode) {
  ...
  if (!databasePath) throw new Error('measurement database path is required');
  (globalThis as { gc?: () => void }).gc?.();
  const startedRss = process.memoryUsage().rss;
  const reopenStarted = performance.now();
  const index = new CacheIndex(databasePath);
  ...
  try {
    ...
    const measurement = spawnSync(
      process.execPath,
      ['--expose-gc', process.argv[1]!, '--measure', databasePath, String(count)],
      { encoding: 'utf8' },
    );
