...
test('redacts PAT, bearer headers, and signed URL values from every persisted message', () => {
  const value = redactSensitiveText(
    'Authorization: Bearer sea_pat_secret https://host/path?token=abc&x-amz-signature=def',
  );
  assert.doesNotMatch(value, /sea_pat_secret|Bearer|token=abc|signature=def/i);
});
