...
describe('getAssetDragIds', () => {
  ...
  it('drags only an unselected card instead of leaking the previous selection', () => {
    expect(getAssetDragIds({ orderedIds, selectedIds: ['one', 'two'], draggedId: 'three' })).toEqual([
      'three',
    ]);
  });
});
...
describe('getDesktopAssetDragBridge', () => {
  it('returns the capability only when the desktop bridge exposes it', () => {
    const startAssetDrag = vi.fn();
    vi.stubGlobal('sekerDesktop', { version: 1, createMediaUrl: vi.fn(), startAssetDrag });
    expect(getDesktopAssetDragBridge()?.startAssetDrag).toBe(startAssetDrag);
    vi.unstubAllGlobals();
  });

  it('keeps browser-only sessions non-draggable', () => {
    vi.stubGlobal('sekerDesktop', { version: 1, createMediaUrl: vi.fn() });
