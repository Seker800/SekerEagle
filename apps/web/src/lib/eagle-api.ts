...
export interface EagleManualTagRef { id: string; name: string; color: string | null }
...
export interface EagleColorAnalysis {
  ...
  swatches: Array<{ rank: number; hex: string; weight: number; labL: number; labA: number; labB: number }>;
}
...
export interface EagleAiTag { id: string; name: string; assetCount: number; pinyin: string; pinyinInitials: string }
...
export interface EagleAssetFilters {
  limit?: number; cursor?: string; smartFolderId?: string; search?: string; formats?: string[];
  manualTagIds?: string[]; aiTagIds?: string[]; rating?: number; minWidth?: number; maxWidth?: number;
  minHeight?: number; maxHeight?: number; createdFrom?: string; createdTo?: string; color?: string;
}
export interface EagleAssetVersion { assetId: string; rowVersion: number }
export interface EagleAssetChanges {
  displayName?: string; rating?: number | null; color?: string | null; description?: string | null; sourceUrl?: string | null;
}
export interface EagleAssetPage {
  items: EagleAssetListItem[];
  nextCursor: string | null;
  colorCoverage: { eligible: number; completed: number; percentage: number; processorVersion: string } | null;
}
export type EagleSmartFolderFilters = Omit<EagleAssetFilters, 'cursor' | 'limit' | 'smartFolderId' | 'color'> & {
  assetColor?: string; tagMatch?: 'ANY' | 'ALL';
};
...
function assetQuery(filters: EagleAssetFilters): string {
  ...
  for (const key of ['cursor', 'smartFolderId', 'search', 'createdFrom', 'createdTo', 'color'] as const) {
...
export async function listEagleAssets(_token: string, filters: EagleAssetFilters = {}, signal?: AbortSignal): Promise<EagleAssetPage> {
  ...
  return { ...result, items: result.items.map(normalizeAsset), colorCoverage: null };
}
export async function listEagleTrash(_token: string, filters: EagleAssetFilters = {}, signal?: AbortSignal): Promise<EagleAssetPage> {
  const result = await api<{ items: EagleAssetListItem[]; nextCursor: string | null }>(`/eagle/trash?${assetQuery(filters)}`, { signal });
  return { ...result, items: result.items.map(normalizeAsset), colorCoverage: null };
}
export async function getEagleAsset(_token: string, id: string, signal?: AbortSignal) {
  return normalizeAsset(await api<EagleAsset>(`/eagle/assets/${encodeURIComponent(id)}`, { signal }));
}
...
function normalizeTag(tag: Partial<EagleManualTag> & EagleManualTagRef & { _count?: { assetLinks: number } }): EagleManualTag {
  return { ...tag, groupId: tag.groupId ?? null, groupIds: tag.groupIds ?? (tag.groupId ? [tag.groupId] : []), isStarred: tag.isStarred ?? false, rowVersion: tag.rowVersion ?? 1, assetCount: tag.assetCount ?? tag._count?.assetLinks ?? 0, pinyin: tag.pinyin ?? tag.name, pinyinInitials: tag.pinyinInitials ?? tag.name };
}
export async function listEagleManualTags(_token: string) { return (await api<Array<Parameters<typeof normalizeTag>[0]>>('/eagle/tags')).map(normalizeTag); }
export async function createEagleManualTag(_token: string, input: { name: string; color?: string | null }) { return normalizeTag(await api<Parameters<typeof normalizeTag>[0]>('/eagle/tags', { method: 'POST', body: JSON.stringify(input) })); }
export async function updateEagleManualTag(_token: string, id: string, input: Partial<Pick<EagleManualTag, 'name' | 'color' | 'groupId' | 'isStarred'>> & { rowVersion: number }) { return normalizeTag(await api<Parameters<typeof normalizeTag>[0]>(`/eagle/tags/${id}`, { method: 'PATCH', body: JSON.stringify(input) })); }
export async function deleteEagleManualTag(_token: string, id: string) { await api(`/eagle/tags/${id}`, { method: 'DELETE' }); return { deletedId: id }; }
export async function listEagleManualTagGroups(_token: string) { return api<EagleManualTagGroup[]>('/eagle/tag-groups'); }
export async function createEagleManualTagGroup(_token: string, input: { name: string; color?: string | null; description?: string | null }) { return api<EagleManualTagGroup>('/eagle/tag-groups', { method: 'POST', body: JSON.stringify(input) }); }
export async function updateEagleManualTagGroup(_token: string, id: string, input: { name?: string; color?: string | null; description?: string | null; rowVersion: number }) { return api<EagleManualTagGroup>(`/eagle/tag-groups/${id}`, { method: 'PATCH', body: JSON.stringify(input) }); }
export async function deleteEagleManualTagGroup(_token: string, id: string) { await api(`/eagle/tag-groups/${id}`, { method: 'DELETE' }); return { deletedId: id }; }
export async function listEagleAiTags(_token: string) { return api<EagleAiTag[]>('/eagle/ai-tags'); }
export async function listEagleSmartFolders(_token: string) { const rows = await api<Array<Omit<EagleSmartFolder, 'queryJson'> & { queryJson?: EagleSmartFolder['queryJson']; query?: EagleSmartFolderFilters }>>('/eagle/smart-folders'); return rows.map((row) => ({ ...row, queryVersion: row.queryVersion ?? 1, queryJson: row.queryJson ?? { version: 1, filters: row.query ?? {} } })); }
export async function createEagleSmartFolder(_token: string, input: EagleSmartFolderFilters & { name: string }) { const { name, ...filters } = input; return api<EagleSmartFolder>('/eagle/smart-folders', { method: 'POST', body: JSON.stringify({ name, query: { version: 1, filters } }) }); }
...
export async function uploadEagleAsset(_token: string, file: File, onProgress: (value: { percent: number }) => void) {
  ...
  try {
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      const { uploadUrl } = await api<{ uploadUrl: string }>(`/eagle/uploads/${session.id}/parts/${partNumber}`, { method: 'POST', body: '{}' });
      const response = await fetch(uploadUrl, { method: 'PUT', body: file.slice(index * session.partSize, Math.min(file.size, (index + 1) * session.partSize)) });
...
export function getEagleAssetContentUrl(id: string) { return `/api/eagle/assets/${encodeURIComponent(id)}/original`; }
export function getEagleRenditionContentUrl(assetId: string, renditionId: string) { return `/api/eagle/assets/${encodeURIComponent(assetId)}/renditions/${encodeURIComponent(renditionId)}`; }
