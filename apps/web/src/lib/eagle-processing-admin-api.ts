...
export interface EagleProcessingSummary {
  worker: { status: 'ONLINE' | 'OFFLINE'; count: number; activeJobCount: number; lastHeartbeatAt: string | null; version: string | null };
  ...
  colorCoverage: { processorVersion: string; eligible: number; completed: number; processing: number; failed: number; percentage: number };
  settings: EagleProcessingSettings;
  refreshedAt: string;
}
export interface EagleProcessingJob {
  id: string; assetReference: string; kind: string; lane: EagleProcessingLane;
  status: EagleProcessingStatus; processorVersion: string; attempts: number; availableAt: string;
...
export function reconcileEagleProcessingJobs(_token: string) { return request<{ scanned: number; created: number; skipped: number; remaining: number }>('reconcile', { method: 'POST', body: '{}' }); }
export function updateEagleProcessingSettings(_token: string, settings: Pick<EagleProcessingSettings, 'mode' | 'nightStart' | 'nightEnd'>) { return request<EagleProcessingSettings>('settings', { method: 'PATCH', body: JSON.stringify(settings) }); }
