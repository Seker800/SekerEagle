...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    throw await errorFromResponse(response, '向量服务请求失败（{{value1}}）');
  }
...
export function retryFailedEagleEmbeddings() {
  return request<{
    retried: number;
  }>('embeddings/retry-failed', { method: 'POST', body: '{}' });
}
